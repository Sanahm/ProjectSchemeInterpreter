...
printHelp()
{
	...
	echo " The <executablefile> will be run on each <testfile> and :"
        echo "	0/ store the standard output of <executablefile> in a file named <testfile>.out "
        echo "	1/ compare the return code value of the program with the expected return code value (PASS, ie 0, or FAIL, ie non 0)"
        echo "	2/ generate a file <testfile>.out containing the output of the program (stdout of the tested program)"
        echo "	3/ compare the expected output (<testfile>.res) with the standard output stdout of the tested program (<testfile>.out)"
...
executeTest()
{
	...
	#ce qu'on a ajoute pour palier les problemes
