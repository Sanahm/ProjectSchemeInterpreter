...
int isspecial(char c) {
	return (c==':'|| (c>='<' && c <='@')||(c>= '-' && c<='/')|| c=='*'|| c=='%'||c=='$'|| c=='!'|| c=='~'|| c=='^'|| c=='_'); 
} 
...
object sfs_read( char *input, uint *here ) { /*here permet de se positionner au niveau de la S_expr cad montre à quel niveau en est la lecture*/
    ...
    if ( input[*here] == '(' ) {
		...
		if( !isspace(input[(*here)+1]) ) return NULL; /* on verifie qu'il y'a un espace derriere la parenthese fermante*/
        if ( input[(*here)] == ')' ){
            (*here)++;
            return nil;
        }
...
object sfs_read_atom( char *input, uint *here ) {/*il faut compléter sfs_read_(pair/atome)*/
	...
	if( input[*here] == '"'){
		(*here)++;		
		while( input[*here] != '"' && *here < strlen(input) ){ /*doit on gerer les erreur depuis ici?*/
			str[i] = input[*here];
			i++;
			(*here)++;
		}
	...
	if(input[*here] =='+' || input[*here] =='-' || isdigit(input[*here])){ 
		str[0] = input[*here];
		(*here)++;
		if(isspace(input[*here]) && !isdigit(input[*here-1])){
			str[1]='\0';
			atom = make_symbol(str); /*teste si +/- est un symbol ou pas  ;; str[0] = input[*here-1]*/
			return atom;
		}
		while( isdigit(input[*here]) && *here < strlen(input)){
			i++;
			str[i] = input[*here];
			(*here)++; 
		}
	...
	if(isalpha(input[*here]) || isspecial(input[*here])){ /* isspecial() à definir retourne 1 si (! % etc)*/
		str[0] = input[*here];
		(*here)++;
		while((isalpha(input[*here]) || isdigit(input[*here])) && *here < strlen(input)){ /* ca c'est un symbol :irud, jdfb9*/
			i++;
			str[i] = input[*here];
			(*here)++;
		}
	...
	if(input[*here] == '#'){
		...
		if(input[*here] == '\\'){
			(*here)++;
			if(isalpha(input[*here]) && isspace(input[*here+1])){
...
object sfs_read_pair( char *stream, uint *i ) {
	...
	if(stream[*i] != ')' && !isspace(stream[*i+1])) pr->this.pair.cdr = sfs_read_pair(stream,i);
